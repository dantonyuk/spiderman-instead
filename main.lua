...
cubes = obj {
    ...
    tak = 'Ты собираешь кубики.';
    inv = function (s)
        if here() == neartable2 then
            inv():del('cubes');
            objs():add('stair');
            return 'Из кубиков ты строишь лесенку...';
        end;
        return 'Кубиков много. Их неудобно нести в руках.';
    end;
    use = function (s, w)
...
kitchen = room {
    ...
    obj = {
        ...
        gobj('nearcooker', 'Чуть дальше {плита}, на которой что-то готовится.'),
...
neartable3 = room {
    ...
    obj = {
        ...
        gobj('nearcooker', 'В углу стоит {плита}. Пахнет чем-то вкусным.'),
...
nearcooker = room {
    ...
    obj = {
        jump_obj(gobj('cooker', '"Может быть забраться на {плиту}?", -- думаешь ты.')),
        gobj('kitchen', '^^{Дверь} в детскую позади тебя.'),
...
cooker = room {
    ...
    obj = {
        mobj(4, 'В сковородке'),
        fall_obj(gobj('nearcooker', '"Под плитой не так пекло. Может {спрыгнуть}?", -- думаешь ты.')),
    }
}
...
heater = room {
    ...
    obj = {
        ...
        gobj('nearcooker', 'Позади тебя стоит {плита}, от которой так вкусно пахнет.')
    }
}

cockroach = dlg {
    nam = 'Таракан';
    dsc = 'Таракан важно повернул к тебе свою усатую голову.';
    obj = {
        ...
        [5] = _phr('Договорились!', '-- Бери, - таракан протянул мне ключ.', [[drop('money'); remove('money'); take('key')]]),
        [6] = phr('До свидания!', 'Пока!', [[pon(); goto('heater');]]),
    };
    exit = function (s, w)
        s:pon(1);
        s:poff(2);
